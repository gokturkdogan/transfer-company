...
export function HeroSearchBar({ onSubmit }: HeroSearchBarProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="rounded-[28px] border border-white/25 bg-white/12 p-1.5 shadow-premium backdrop-blur-2xl">
        <div className="grid grid-cols-1 gap-1 rounded-[22px] bg-card/95 p-1.5 sm:grid-cols-2 lg:flex lg:items-center lg:gap-0">
          <LocationSegment
            icon={PlaneLanding}
            label={t("airport")}
          ...
          <button
            ...
            className={cn(
              "group relative flex h-12 shrink-0 cursor-pointer items-center justify-center gap-2",
              "overflow-hidden rounded-2xl bg-gold-gradient px-6 text-sm font-bold tracking-tight text-ink",
              "shadow-gold transition-all duration-300 hover:brightness-110 active:scale-[0.98]",
              "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2",
              "disabled:cursor-not-allowed disabled:opacity-45 disabled:shadow-none",
              "sm:col-span-2 lg:col-span-1 lg:h-[52px] lg:w-auto",
            )}
          >
...
function TripTypeToggle({
  ...
  return (
    <div
      role="radiogroup"
      className="inline-flex items-center gap-1 rounded-full border border-white/20 bg-white/10 p-1 backdrop-blur-xl"
    >
      {options.map((option) => {
